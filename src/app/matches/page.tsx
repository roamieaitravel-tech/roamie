...
export default function MatchesPage(): React.JSX.Element {
  ...
  const fetchMatches = async (destinationValue: string, start: string, end: string, maxBudget: number): Promise<void> => {
  ...
  const loadConnections = async (currentUserId: string): Promise<void> => {
    ...
    const otherUserIds = Array.from(
      new Set(
        matchesData.flatMap((match: { user_id_1: string | null; user_id_2: string | null }) => [match.user_id_1 as string, match.user_id_2 as string]).filter((id: string) => id !== currentUserId)
      )
    );
    ...
    const profileById = new Map<string, ProfileSummary>(profilesData?.map((item: ProfileSummary) => [item.user_id, item]) ?? []);

    const connections: ConnectionItem[] = matchesData.map((match: { id: string | null; trip_id: string | null; user_id_1: string | null; user_id_2: string | null; status: string | null; updated_at: string | null }) => {
  ...
  useEffect(() => {
    const loadUser = async (): Promise<void> => {
      ...
      let currentBudgetMax = budgetMax;

      if (profileData) {
        setProfile(profileData);
        if (profileData.preferred_destinations?.length) {
          currentDest = profileData.preferred_destinations[0];
          setDestination(currentDest);
        }
        if (profileData.budget_range) {
          currentBudgetMax = profileData.budget_range.max;
          setBudgetMax(currentBudgetMax);
        }
      }

      await fetchMatches(currentDest, startDate, endDate, currentBudgetMax);
  ...
  const handleConnect = async (profileItem: MatchProfile): Promise<void> => {
  ...
  const handleAccept = async (item: ConnectionItem): Promise<void> => {
    try {
      const supabase = createClient();
      ...
      setErrorMessage("Unable to accept match request.");
    }
  };

  const handleDecline = async (item: ConnectionItem): Promise<void> => {
    try {
      const supabase = createClient();
      ...
      setErrorMessage("Unable to decline match request.");
    }
  };

  const handleCancel = async (item: ConnectionItem): Promise<void> => {
    try {
      const supabase = createClient();
      ...
      setErrorMessage("Unable to cancel match request.");
    }
  };
