...
function calculateOverlapScore(requestStart: string, requestEnd: string, candidateStart: string, candidateEnd: string) {
...
function calculateVibeScore(sharedTags: string[], sameStyle: boolean, budgetOverlap: boolean, overlapScore: number) {
...
export async function GET(request: Request) {
  try {
    ...
    const candidateUserIds = Array.from(new Set(tripsData?.map((trip) => trip.user_id) ?? []));
    ...
    const profileByUserId = new Map(profilesData?.map((profile) => [profile.user_id, profile]) ?? []);
