...
export async function POST(request: Request) {
  try {
    ...
    const currentProfile = matchScoreData?.find((item) => item.user_id === currentUser.id) as {
      vibe_tags?: string[];
      travel_style?: string[];
      budget_range?: { min: number; max: number };
    };
    const matchedProfile = matchScoreData?.find((item) => item.user_id === body.matchedUserId) as {
      vibe_tags?: string[];
      travel_style?: string[];
      budget_range?: { min: number; max: number };
    };

    const sharedTags = currentProfile?.vibe_tags?.filter((tag) => matchedProfile?.vibe_tags?.includes(tag)) ?? [];
